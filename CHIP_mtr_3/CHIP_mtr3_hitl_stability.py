...
def metrics(df_baseline: pd.DataFrame, df_sample: pd.DataFrame) -> dict:
    ...
    Args:
        df_baseline (pandas.DataFrame): Baseline dataset containing model inputs and score
        df_sample (pandas.DataFrame): Sample (prod) dataset containing model inputs and score

    Returns:
        dict: Merged dictionary of Stability and Drift metrics
    """
...
if __name__ == "__main__":
    ...
    # 3. Load test data
    try:
        df_b = pd.read_json('mtr_3_baseline.json', orient='records')
        df_c = pd.read_json('mtr_3_comparator.json', orient='records')
    except Exception as e:
