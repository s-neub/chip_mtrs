"""OOTB ModelOp Center model to compute bias (disparity) metrics on protected classes"""
...
def metrics(dataframe: pandas.DataFrame) -> dict:
    """A function to compute bias (disparity) metrics on sample prod data
    ...
    Returns:
        (dict): Bias (disparity) metrics for each protected class
    """
    ...
    yield bias_monitor.compute_bias_metrics(pre_defined_test="aequitas_bias")
